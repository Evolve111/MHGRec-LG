...
GPT_MODEL = os.getenv("LLM_MODEL", "deepseek-r1:latest")

# Local requests to Ollama are free; set costs to zero
prompt_cost_1k, completion_cost_1k = 0.0, 0.0
...
def set_gpt_pricing(prompt_price_per_1k: float, completion_price_per_1k: float):
    global prompt_cost_1k, completion_cost_1k
    prompt_cost_1k = float(prompt_price_per_1k)
    completion_cost_1k = float(completion_price_per_1k)

def get_llm_usage_counters():
    return {
        "prompt_tokens": GLOBAL_PROMPT_TOKENS,
        "completion_tokens": GLOBAL_COMPLETION_TOKENS,
        "call_count": GLOBAL_CALL_COUNT
    }

def reset_llm_usage_counters():
    global GLOBAL_PROMPT_TOKENS, GLOBAL_COMPLETION_TOKENS, GLOBAL_CALL_COUNT
    GLOBAL_PROMPT_TOKENS = 0
    GLOBAL_COMPLETION_TOKENS = 0
    GLOBAL_CALL_COUNT = 0

def get_gpt_completion(dialogs, gpt_model=GPT_MODEL, client=None, tools=None, tool_choice=None, temperature=0.6, max_tokens=1000, top_p=1.0):
    max_retries = 5

    # Prefer client-provided base_url/model if available
    base_url = getattr(client, 'base_url', OLLAMA_BASE_URL) if client is not None else OLLAMA_BASE_URL
    model = getattr(client, 'model', gpt_model) if client is not None else gpt_model
    api_key = getattr(client, 'api_key', os.getenv("LLM_API_KEY", ""))
    ...
    for i in range(max_retries):
        try:
            ...
            try:
                ...
                ct = 0
            global GLOBAL_PROMPT_TOKENS, GLOBAL_COMPLETION_TOKENS, GLOBAL_CALL_COUNT
            GLOBAL_PROMPT_TOKENS += pt
            GLOBAL_COMPLETION_TOKENS += ct
            GLOBAL_CALL_COUNT += 1
            this_cost = prompt_tokens/1000*prompt_cost_1k + completion_tokens/1000*completion_cost_1k
            return SimpleMessage(role=msg_dict.get("role"), content=msg_dict.get("content", "")), this_cost
...
def chat_completion_request(messages, tools=None, tool_choice=None, model=GPT_MODEL):
    # Simple wrapper using local Ollama OpenAI-compatible endpoint
    json_data = {"model": model, "messages": messages}
    try:
        api_key = os.getenv("LLM_API_KEY", "")
        headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
        resp = requests.post(f"{OLLAMA_BASE_URL}/chat/completions", json=json_data, headers=headers, timeout=60)
        resp.raise_for_status()
        response = resp.json()
        usage = response.get("usage", {})
        prompt_tokens = usage.get("prompt_tokens", 0)
        completion_tokens = usage.get("completion_tokens", 0)
        this_cost = prompt_tokens/1000*prompt_cost_1k + completion_tokens/1000*completion_cost_1k
        msg_dict = response.get("choices", [{}])[0].get("message", {})
        return SimpleMessage(role=msg_dict.get("role"), content=msg_dict.get("content", "")), this_cost
    except Exception as e:
...
def get_relationship(G, dataset_string, node1, node2):
    # Support Bookcrossing, Amazons, Movielens datasets with minimal semantics
    ds = (dataset_string or '').lower()
    assert ds in ('bookcrossing', 'amazons', 'movielens'), f"Unsupported dataset '{dataset_string}'"
    relationships = {
        'U-B': 'Rates',
        'B-U': 'Is Rated By',
        'U-I': 'Rates',
...
def l2_normalize_rows(arr):
    """L2-normalize each row of a numpy array; safe for zeros."""
...
def temp_edge_type_lookup(dataset_string):
    # Return dataset-specific edge type lookup used by arch constructors
    ds = (dataset_string or '').lower()
    assert ds in ('bookcrossing', 'amazons', 'movielens'), f"Unsupported dataset '{dataset_string}'"
    if ds == 'bookcrossing':
        # Adjacency list order in runtime: [forward(0), reverse(1), identity(2), zero(3)]
        # Map semantic types to indices consistent with constructed list
        return {
            'UB': 0,
            'BU': 1,
            'I': 2,

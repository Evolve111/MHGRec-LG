    ...
    def forward(self, x, adjs, idxes_seq, idxes_res, connection_dict):
        ...
        for i in range(self.n_step): # 4 or 6
            ...
            try:
                ...
                import logging
                logging.exception(f"Residual aggregation failed at step {i}; offset={offset}; falling back to zeros: {e}")
                resi = torch.zeros_like(states[i])
    ...
    def forward(self, node_feats, node_types, adjs, idxes_seq, idxes_res, connection_dict): 
        ...
        for i in range(len(node_feats)):
            ...
            if hasattr(x, 'is_sparse') and x.is_sparse:
                out_i = torch.sparse.mm(x, self.ws[i].weight.t())
                if self.ws[i].bias is not None:
                    out_i = out_i + self.ws[i].bias
            else:
                out_i = self.ws[i](x)
            hid[node_types == i] = out_i
